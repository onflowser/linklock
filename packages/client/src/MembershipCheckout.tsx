...
export function MembershipCheckout({
    ...
    const ownedTargetMembership = ownedMemberships?.find(
        (membership) => membership.adminAddress === adminAddress && (membershipDefinition && membership.id === membershipDefinition!.id)
    );
    const [checkoutStep, setCheckoutStep] = useState(CheckoutStep.PREVIEW);

    useEffect(() => {
        if (ownedTargetMembership && ownedTargetMembership.id === membershipDefinition!.id) {
